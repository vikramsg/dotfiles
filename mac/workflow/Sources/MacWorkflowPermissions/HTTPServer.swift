...
final class HTTPServer {
    ...
    private func route(_ request: HTTPRequest, completion: @escaping (HTTPResponse) -> Void) {
        ...
        do {
            ...
            case ("POST", "/v1/permissions/accessibility/request"):
                completion(.ok(["accessibility": PermissionService.accessibility(prompt: true)]))
            case ("POST", "/v1/permissions/screen-recording/request"):
                completion(.ok(["screen_recording": PermissionService.screenRecording(prompt: true)]))
