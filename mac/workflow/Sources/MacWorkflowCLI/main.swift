...
func usage() -> Never {
    ...
    Usage:
      ...
      mac-workflow windows <bundle-id>
      mac-workflow screens
      mac-workflow launch <bundle-id>
      mac-workflow frame <window-id> <x> <y> <width> <height>
      mac-workflow focus <window-id>
      mac-workflow unminimize <window-id>
      mac-workflow overlay <image-path> [timeout-seconds]
      mac-workflow overlays
      mac-workflow hide-overlays
      mac-workflow keystroke <key> [modifier ...]
      mac-workflow click <left|right> <x> <y>
      mac-workflow drag <from-x> <from-y> <to-x> <to-y> [duration]
      mac-workflow shelves
      mac-workflow shelf <directory>
      mac-workflow close-shelf <id>
      mac-workflow screenshot [--display <id>] [--path <png-path>]
    """)
    exit(2)
}
...
do {
    ...
    case "permissions":
        data = try request(method: "GET", path: "/v1/permissions")
    case "request-accessibility":
        data = try request(method: "POST", path: "/v1/permissions/accessibility/request")
    case "request-screen-recording":
        data = try request(method: "POST", path: "/v1/permissions/screen-recording/request")
    case "applications":
    ...
    fputs("mac-workflow: \(error.localizedDescription)\n", stderr)
    exit(1)
}
