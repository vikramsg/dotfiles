...
require("lazy").setup({
	...
	-- yazi
	{
		...
		keys = {
			...
			{
				-- Open in the current working directory and toggle hidden
				"<leader>cW",
				function()
					require("yazi").yazi(nil, vim.fn.getcwd())
					-- Wait a tiny bit for Yazi to load, then send '.' to toggle hidden
					vim.defer_fn(function()
						vim.api.nvim_feedkeys(".", "n", false)
					end, 100)
				end,
				desc = "Open Yazi in project root (showing hidden/ignored)",
			},
			{
				"<c-up>",
				"<cmd>Yazi toggle<cr>",
