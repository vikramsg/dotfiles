  ...
  const configText = JSON.stringify(
    {
      plugin: [
        ...
        "./plugins/orchestration-state.js",
      ],
    },
    null,
    2,
...
async function withHomeAuth<T>(
  fn: (home: string, authFile: string) => Promise<T>,
): Promise<T> {
...
async function writeScenarioRecipe(
  root: string,
...
async function readArtifactFiles(sandboxRoot: string) {
  ...
  return {
    ...
    metadata: JSON.parse(
      await readFile(path.join(output, "metadata.json"), "utf8"),
    ),
    stdout: await readFile(path.join(output, "stdout.txt"), "utf8"),
    stderr: await readFile(path.join(output, "stderr.txt"), "utf8"),
    rawStatus: await readFile(
      path.join(output, "opencode-exit-status.txt"),
      "utf8",
    ),
...
describe("cli-v2", () => {
  it("creates the sandbox directory layout with typed paths", async () => {
    ...
    expect(layout.opencodeConfigDir).toBe(
      path.join(layout.configHome, "opencode"),
    );
    expect(layout.pluginDir).toBe(
      path.join(layout.opencodeConfigDir, "plugins"),
    );
    ...
    expect(layout.sandboxAuthFile).toBe(
      path.join(layout.dataHome, "opencode", "auth.json"),
    );
  ...
  it("prepares a single-agent sandbox from real source files", async () => {
    ...
    expect(await readFile(prepared.layout.sandboxConfigFile, "utf8")).toBe(
      files.configText,
    );
    expect(await readFile(prepared.layout.sandboxAuthFile, "utf8")).toBe(
      await readFile(files.sourceAuthFile, "utf8"),
    );
    expect(await readFile(pluginFile, "utf8")).toBe(
      await readFile(files.pluginA, "utf8"),
    );
    await expect(
      readFile(path.join(prepared.layout.pluginDir, "stop-marker.js"), "utf8"),
    ).rejects.toThrow();
    await expect(
      readFile(path.join(prepared.layout.pluginDir, "ignored.ts"), "utf8"),
    ).rejects.toThrow();
    expect(await readFile(agentFile, "utf8")).toBe(
  ...
  it.each([
    ...
    async (agentName) => {
      ...
      await expect(
        readFile(
          path.join(dest, "config", "opencode", "agents", "evil.md"),
          "utf8",
        ),
      ).rejects.toThrow();
    },
  ...
  it("rejects absolute local plugin paths before copying sandbox files", async () => {
    ...
    await expect(
      ...
      `Absolute local plugin paths are not supported in sandbox config: ${plugin}`,
    );

    const sandboxRoot = path.resolve(dest);
    await expect(
      readFile(
        path.join(sandboxRoot, "config", "opencode", "opencode.json"),
        "utf8",
      ),
    ).rejects.toThrow();
    await expect(
      readFile(
        path.join(sandboxRoot, "config", "opencode", "plugins", "absolute.js"),
        "utf8",
      ),
    ).rejects.toThrow();
    await expect(
  ...
  it("rejects traversal plugin entries before copying any plugin files", async () => {
    ...
    await writeFile(
      config,
      JSON.stringify({ plugin: ["./plugins/local.js", "../evil.js"] }, null, 2),
    );
    ...
    await expect(
      ...
      "Configured local plugin escapes sandbox plugin directory: ../evil.js ->",
    );

    const sandboxRoot = path.resolve(dest);
    await expect(
      readFile(path.join(sandboxRoot, "config", "evil.js"), "utf8"),
    ).rejects.toThrow();
    await expect(
      readFile(
        path.join(sandboxRoot, "config", "opencode", "plugins", "local.js"),
        "utf8",
      ),
    ).rejects.toThrow();
  });
  ...
  it("runs a generic single-agent sandbox with prompt text and output artifacts", async () => {
    ...
    ).toBe(files.configText);
    expect(
      (
        await readdir(path.join(sandboxRoot, "config", "opencode", "plugins"))
      ).sort(),
    ).toEqual(["orchestration-state.js"]);
    expect(
      await readFile(
        path.join(
          sandboxRoot,
    ...
    ).toBe(await readFile(files.sourceAgentFile, "utf8"));
    expect(captured.stdout).toBe(
      "[opencode stdout begin]\nfake stdout\n[opencode stdout end]\n",
    );
    expect(captured.stderr).toBe(
      "[opencode stderr begin]\nfake stderr\n[opencode stderr end]\n",
    );
    ...
    expect(artifacts.metadata.sourceAuthFile).toContain(
      path.join(".local", "share", "opencode", "auth.json"),
    );
    expect(artifacts.metadata.sandboxAuthFile).toBe(
      path.join(sandboxRoot, "data", "opencode", "auth.json"),
    );
  ...
  it.each([
    ...
    "requires exactly one prompt source for %s prompt options",
    async (_name, promptArgs) => {
      const orig = await tempDir();
      const dest = await tempDir("cli-v2-sandbox-");
      const files = await writeSourceFiles(orig);
      const captured = captureIO();
      ...
      await expect(
        readFile(
          path.join(path.resolve(dest), "config", "opencode", "opencode.json"),
          "utf8",
        ),
      ).rejects.toThrow();
    },
  ...
  it("runs hello-world using the self-contained fixture agent", async () => {
    ...
    const status = await withHomeAuth(() =>
      runCli(["node", "cli-v2", "hello-world", "--dest", dest], captured.io, {
        env: {
          ...process.env,
          PATH: `${bin}${path.delimiter}${process.env.PATH ?? ""}`,
        },
    ...
    ).toBe(await readFile(fixtureAgent, "utf8"));
  });
  ...
  it("runs a scenario recipe and copies the fixture worktree", async () => {
    ...
    expect(
      await readFile(path.join(sandboxRoot, "worktree", "README.md"), "utf8"),
    ).toBe(
  ...
  it("fails cleanly before sandbox creation when a scenario agent file is missing", async () => {
    const root = await tempDir();
    const missingAgentPath = path.join(
      opencodeRoot,
      "sandbox",
      "cli-v2",
      "fixtures",
      "agents",
      "missing-agent-for-preflight.md",
    );
    ...
    const child = spawn(
      process.execPath,
      ["sandbox/cli-v2/index.ts", "scenario", scenarioDir, "--dest", dest],
      {
        cwd: opencodeRoot,
        stdio: ["ignore", "pipe", "pipe"],
      },
    ...
    expect(stdout).toBe("");
    expect(stderr).toBe(
      `Source agent file does not exist: ${missingAgentPath}\n`,
    );
    await expect(stat(dest)).rejects.toThrow();
  });
  ...
  it.each([
    [
      "absolute fixtureDir",
      (root: string) => path.join(root, "outside-worktree"),
      "Scenario fixtureDir must be relative",
    ],
    [
      "traversal fixtureDir",
      () => "../outside-worktree",
      "Scenario fixtureDir escapes scenario directory",
    ],
  ])(
    "fails cleanly before sandbox preparation for %s",
    async (_name, fixtureDir, message) => {
      const root = await tempDir();
      const scenarioDir = await writeScenarioRecipe(root, {
        fixtureDir: fixtureDir(root),
      });
      const dest = path.join(root, "dest-that-should-not-be-created");
      const captured = captureIO();
