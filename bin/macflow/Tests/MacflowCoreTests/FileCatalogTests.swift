...
@Suite struct FileCatalogTests {
    @Test func testCatalogFiltersUnsupportedEntriesAndSortsNewestFirst() {
        let candidates = [
    ...
    @Test func testCatalogReturnsAllSupportedFilesWhenBelowMaximum() {
        let candidates = [
            ScreenshotCandidate(url: URL(fileURLWithPath: "/shots/older.png"), modificationDate: Date(timeIntervalSince1970: 10), regularFile: true),
