...
from bin.screenshot_sync.screenshot_sync.cli import PLIST_LABEL, SyncConfig
...
def test_config_loading(mock_config_file, monkeypatch):
    monkeypatch.setattr("bin.screenshot_sync.screenshot_sync.cli.CONFIG_FILE", mock_config_file)
...
def test_uv_tool_install_and_path_verification():
    tool_member_path = Path(__file__).resolve().parents[1]
...
def test_plist_generation(mock_run, mock_config_file, monkeypatch, tmp_path):
    ...
    assert "<string>test-vm</string>" not in content
