...
def test_ctx_sql_installs_temporary_views(tmp_path: Path) -> None:
...
def test_ctx_sql_allows_select_star_from_ctx_sessions(tmp_path: Path) -> None:
    db_path = create_opencode_db(tmp_path / "opencode.db")
...
def test_ctx_sql_rejects_direct_non_history_base_table_read(tmp_path: Path) -> None:
    db_path = create_opencode_db(tmp_path / "opencode.db")
...
def test_ctx_sql_rejects_direct_non_history_base_table_count(tmp_path: Path) -> None:
    db_path = create_opencode_db(tmp_path / "opencode.db")
...
def test_ctx_sql_rejects_cte_shadowing_ctx_view_names(tmp_path: Path) -> None:
    db_path = create_opencode_db(tmp_path / "opencode.db")

    with pytest.raises(ValueError, match="shadow ctx views"):
        run_ctx_sql(
            OpenCodeRepository(db_path),
            "WITH ctx_sessions AS (SELECT * FROM account) SELECT COUNT(*) FROM ctx_sessions",
        )


def test_ctx_sql_allows_non_shadowing_ctes_over_ctx_views(tmp_path: Path) -> None:
    db_path = create_opencode_db(tmp_path / "opencode.db")

    rows = run_ctx_sql(
        OpenCodeRepository(db_path),
        "WITH sessions AS (SELECT session_id FROM ctx_sessions) SELECT COUNT(*) AS sessions FROM sessions",
    )

    assert rows == [{"sessions": 2}]


def test_ctx_sql_views_include_files_touched(tmp_path: Path) -> None:
    db_path = create_opencode_db(tmp_path / "opencode.db")
    rows = run_ctx_sql(
        OpenCodeRepository(db_path),
        """
...
def test_ctx_sql_maps_native_event_columns_and_payload_time(tmp_path: Path) -> None:
    db_path = create_opencode_db(tmp_path / "opencode.db")
    rows = run_ctx_sql(
        OpenCodeRepository(db_path),
        """
    ...
    assert rows == [
        {"provider_session_id": "s-primary", "event_type": "tool.invocation", "time_created": 1704067200004}
    ]


def test_ctx_sql_falls_back_to_payload_session_id_for_native_events(tmp_path: Path) -> None:
    db_path = create_opencode_db(tmp_path / "opencode.db")
    rows = run_ctx_sql(
        OpenCodeRepository(db_path),
...
def test_ctx_sql_rejects_mutation(tmp_path: Path) -> None:
    db_path = create_opencode_db(tmp_path / "opencode.db")
