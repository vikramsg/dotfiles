...
from ocint.daemon.service import Job, JobStage, JobState, WorkRequest
...
class ControlRepository:
    ...
    def checkpoint(self, job_id: str, stage: JobStage, **values: str | bool) -> Job:
        columns: dict[str, str | int] = {"stage": stage.value, "updated_at": datetime.now(UTC).isoformat()}
        allowed = {
            "session_id",
            "server_url",
            "worktree_path",
        ...
        for name, value in values.items():
            ...
            columns[name] = int(value) if isinstance(value, bool) else value
        with self.engine.begin() as connection:
