...
receives no publication credentials. Git uses only the explicitly supplied SSH
agent socket and configured commit identity. The GitHub token is used only by
the idempotent pull-request client.
